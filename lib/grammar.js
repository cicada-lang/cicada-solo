...
function exp() {
    return new rule_1.Rule("exp", {
        ...
        "ap": [exp, "(", predefined_1.ap(ptc.non_empty_list, arg), ")"],
        "cl": ["class", "{", scope, "}"],
        "cl_empty": ["class", "{", "}"],
        "obj": ["{", scope, "}"],
        "obj_empty": ["{", "}"],
...
const exp_matcher = AST.Node.matcher_with_span(span => [
    "exp", {
        ...
        "ap": ([target, , arg_list, _]) => new Exp.Ap(exp_matcher(target), ptc.non_empty_list_matcher(arg_matcher)(arg_list)),
...
function arg() {
    return new rule_1.Rule("arg", {
        "arg": [exp],
        "arg_comma": [exp, ","],
    });
}
const arg_matcher = AST.Node.matcher_with_span(span => [
    "arg", {
