...
function eliminate_ap(env, target, args) {
    if (target instanceof Value.TheNeutral) {
        ...
        if (the.t instanceof Value.Pi) {
            ...
            return new Value.TheNeutral(evaluate(scope_env, pi.return_type), new Neutral.Ap(the.value, args.map(arg => evaluate(env, arg))));
        }
        else {
            throw new Err.Report([
                "eliminate_ap fail\n" +
...
function eliminate_transport(env, target, motive, base) {
    if (target instanceof Value.TheNeutral) {
        ...
        if (the.t instanceof Value.Equation) {
            ...
            return new Value.TheNeutral(eliminate_ap(equation_env, motive_value, [rhs]), new Neutral.Transport(the.value, motive_value, base_value));
        }
        else {
            throw new Err.Report([
...
function eliminate_dot(env, target, field_name) {
    if (target instanceof Value.TheNeutral) {
        let the = target;
        return new Value.TheNeutral(eliminate_dot(env, the.t, field_name), new Neutral.Dot(the.value, field_name));
    }
