...
function infer(env, exp) {
    try {
        if (exp instanceof Exp.Var) {
            ...
            check_1.check(env, lhs, t_value);
            check_1.check(env, rhs, t_value);
            return new Value.Type();
        }
        else if (exp instanceof Exp.Same) {
            let { t, value } = exp;
            return new Value.Equation(evaluate_1.evaluate(env, t), value, value, env);
        }
        else if (exp instanceof Exp.Transport) {
            let { equation, motive, base } = exp;
            let equation_value = evaluate_1.evaluate(env, equation);
            if (equation_value instanceof Value.Equation) {
                ...
                if (motive_type instanceof Value.Pi) {
                    ...
                    let motive_value = evaluate_1.evaluate(env, motive);
                    check_1.check(env, base, evaluate_1.eliminate_ap(equation_env, motive_value, [lhs]));
                    return evaluate_1.eliminate_ap(equation_env, motive_value, [rhs]);
                }
                else {
                    throw new Err.Report([
...
function infer_ap(env, target, args) {
    ...
    if (t_infered instanceof Value.Pi) {
        ...
        try {
            ...
            if (error instanceof Err.Report) {
                throw error.prepend("infer_ap fail on Value.Pi\n" +
