...
exports.check = check;
function check_obj(env, scope, t) {
    if (t instanceof Value.Cl) {
        ...
        for (let [name, the] of cl.defined) {
            ...
            check(env, v, the.t);
            let v_value = evaluate_1.evaluate(env, v);
            equivalent_1.equivalent(v_value, the.value);
        }
        ...
        for (let [name, entry] of cl.scope.named_entries) {
            ...
            if (entry instanceof Scope.Entry.Let) {
                let { value: d } = entry;
                let t_value = infer_1.infer(env, v);
                let v_value = evaluate_1.evaluate(env, v);
                let d_value = evaluate_1.evaluate(scope_env, d);
                equivalent_1.equivalent(d_value, v_value);
...
function check_same(env, same_type, same_value, t) {
    if (t instanceof Value.Equation) {
        let { t: equation_type_value, lhs, rhs, equation_env } = t;
        check(env, same_type, new Value.Type());
        let same_type_value = evaluate_1.evaluate(env, same_type);
        check(env, same_value, same_type_value);
        let same_value_value = evaluate_1.evaluate(env, same_value);
        equivalent_1.equivalent(same_type_value, equation_type_value);
        let lhs_value = evaluate_1.evaluate(equation_env, lhs);
        let rhs_value = evaluate_1.evaluate(equation_env, rhs);
        equivalent_1.equivalent(same_value_value, lhs_value);
        equivalent_1.equivalent(same_value_value, rhs_value);
    }
    else {
        throw new Err.Report([
...
exports.check_same = check_same;
function check_fn(env, scope, body, t) {
    if (t instanceof Value.Pi) {
        ...
        for (let i = 0; i < scope.arity; i++) {
            ...
            subtype_1.subtype(fn_arg_type_value, pi_arg_type_value);
        ...
        check(local_env, body, evaluate_1.evaluate(scope_env, pi.return_type));
    }
    else {
        throw new Err.Report([
            "expecting pi type\n" +
