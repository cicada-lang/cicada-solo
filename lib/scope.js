...
(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "./value", "./evaluate", "./check", "./infer", "./report"], factory);
    }
    ...
    exports.scope_check_args = scope_check_args;
    function scope_check(scope, env, effect = (name, the) => { }) {
        var local_env = env;
        for (let [name, entry] of scope.named_entries) {
            if (entry instanceof Entry.Let) {
                let { value } = entry;
                let t_value = infer_1.infer(local_env, value);
                let the = {
                    t: t_value,
