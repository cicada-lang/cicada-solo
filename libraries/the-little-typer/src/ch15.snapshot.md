...
def front(E: Type, n: Nat, vector: Vector(E, add1(n))): E =
  ...
  let aux: motive(add1(n), vector) =
    vector_ind(
      ...
      (prev, head, tail, almost) => (j, equal) => head)
...
def rest(E: Type, n: Nat, vector: Vector(E, add1(n))): Vector(E, n) =
  ...
  let aux: motive(add1(n), vector) =
    vector_ind(
      ...
      // NOTE There should be no difference when `prev` is used.
...
the((Nat, Nat, Nat, Nat) -> (Nat, Nat) -> Nat, (j, head, tail, almost) => (j, equal) => tail)
the((Nat, Nat, Nat, Nat) -> (Nat, Nat) -> Nat, (prev, head, tail, almost) => (j, equal) => tail)
```
...
``` cicada
def Dec(X: Type): Type = Either(X, (X) -> Absurd)
```
