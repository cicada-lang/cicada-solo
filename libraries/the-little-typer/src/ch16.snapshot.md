...
def nat_equal_decide_motive(target: Nat): Type =
  (j: Nat) -> Decide(Equal(Nat, target, j))

def nat_equal_decide_base: nat_equal_decide_motive(0) =
  (j) => zero_equal_decide(j)

def nat_equal_decide_step_motive(prev: Nat, target: Nat): Type =
  Decide(Equal(Nat, add1(prev), target))

def nat_equal_decide_step(
  prev: Nat,
  ...
  (j) => nat_ind(
    j,
    nat_equal_decide_step_motive(prev),
    base,
    step)
