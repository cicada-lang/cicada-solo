...
export function readback(value: Value.Value): Exp.Exp {
  if (value instanceof Value.Type) {
    ...
    return new Exp.Pi(new Scope.Scope(named_entries), readback(evaluate(new_scope_env, return_type)))
  }

  else if (value instanceof Value.Fn) {
    let { scope, body, scope_env } = value
    let { named_entries, new_scope_env } = scope_readback(scope, scope_env)
    return new Exp.Fn(new Scope.Scope(named_entries), readback(evaluate(new_scope_env, body)))
  }

  else if (value instanceof Value.FnCase) {
    let { cases } = value
    throw new Exp.FnCase(cases.map(fn => {
    ...
    throw new Exp.Cl(new Scope.Scope([
...
function scope_readback(
  scope: Scope.Scope,
  scope_env: Env.Env,
): { named_entries: Array<[string, Scope.Entry.Entry]>, new_scope_env: Env.Env } {
  let named_entries: Array<[string, Scope.Entry.Entry]> = []
