...
export interface ReturnedApInsertionEntry {
  arg_t: Value
  returned_arg: Value
  not_yet_value: Exps.NotYetValue
}

export abstract class ReturnedInserter {
  arg_t: Value
  ret_t_cl: Closure
  ...
  insert_returned_fn(ctx: Ctx, exp: Exp): Core {
    ...
    return new Exps.ReturnedFnCore(fresh_name, core)
  }

  abstract insert_returned_ap(
    ctx: Ctx,
    target: Core,
    arg: Exp,
    entries: Array<ReturnedApInsertionEntry>
  ): { t: Value; core: Core }

  abstract solve_returned_ap(ctx: Ctx, arg: Exp): Solution
}
