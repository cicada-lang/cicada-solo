...
const arg_entry_matcher = ptc.Node.matcher_with_span<Exp.Exp>((span) => [
  "arg_entry",
  {
...
const top_entry_matcher = ptc.Node.matcher_with_span<Top.Top>((span) => [
  "top_entry",
  {
    named_entry: ([named_entry]) =>
...
export const top_list = new ptc.Rule("top_list", {
  top_entry_list: [ptc.ap(ptc.non_empty_list, top_entry)],
})

export const top_list_matcher = ptc.Node.matcher_with_span<Array<Top.Top>>(
  (span) => [
    "top_list",
    {
...
const scope_matcher: (
  tree: ptc.Tree
) => Scope.Scope = ptc.Node.matcher_with_span<Scope.Scope>((span) => [
  "scope",
  {
    named_entries: ([named_entries]) =>
      new Scope.Scope(
        ptc.non_empty_list_matcher(named_entry_matcher)(named_entries)
      ),
  },
])
...
const named_entry_matcher: (
  tree: ptc.Tree
) => [string, Scope.Entry.Entry] = ptc.Node.matcher_with_span<
  [string, Scope.Entry.Entry]
>((span) => [
  "named_entry",
  {
    let: ([name, , exp]) => [
...
const exp_matcher: (tree: ptc.Tree) => Exp.Exp = ptc.Node.matcher_with_span<
  Exp.Exp
>((span) => [
  "exp",
  {
    ...
    fn_case: ([, fn_case_clause_list, _]) =>
      new Exp.FnCase(
        ptc.non_empty_list_matcher(fn_case_clause_matcher)(fn_case_clause_list)
      ),
...
const fn_case_clause_matcher = ptc.Node.matcher_with_span<Exp.Fn>((span) => [
  "fn_case_clause",
  {
    case: ([, scope, , , body]) =>
      new Exp.Fn(scope_matcher(scope), exp_matcher(body)),
