...
export function assert_json_boolean(data: Json): boolean {
  ...
  throw new Error(
    `Expecting data to be boolean instead of: ${JSON.stringify(data)}.\n`
  )
}
...
export function assert_json_object(data: Json): { [key: string]: Json } {
  if (data === null)
    throw new Error(
      `Expecting data to be object instead of null: ${JSON.stringify(data)}.\n`
    )
  if (data instanceof Array)
    throw new Error(
      `Expecting data to be object instead of array: ${JSON.stringify(data)}.\n`
    )
