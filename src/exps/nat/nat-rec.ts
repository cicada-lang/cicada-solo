...
export class NatRec extends Exp {
  ...
  infer(ctx: Ctx): { t: Value; core: Core } {
    ...
    const fresh_name = "nat_rec_target_nat_" + nanoid().toString()
    const motive_core = new Cores.Pi(fresh_name, new Cores.Nat(), base_t_core)
    const step_core = check(ctx, this.step, nat_rec_step_t(inferred_base.t))
...
function nat_rec_step_t(base_t: Value): Value {
  return evaluate(
