...
In the [Equal Utilities](../equality/01-equal-utilities.md) chapter,
...
function add_add1_commute(
  ...
  return induction (x) {
    (x) => Equal(Nat, add(x, add1(y)), add1(add(x, y)))
    case zero => refl
...
proves the commutative property of addition *for all `Nat`*,
...
But you see,
when we prove in programming language,
...
>
> [**Thus spake Vladimir Voevodsky**](https://readonly.link/articles/github.com/xieyuheng/inner/-/persons/vladimir-voevodsky/how-i-became-interested-in-foundations-of-mathematics.md)
